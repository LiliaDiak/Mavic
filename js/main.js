$(function(){

$('.products__slider').slick({
prevArrow:'<button class="slider-btn slider-btn__left"><svg width="10" height="18" viewBox="0 0 10 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9.21839 1L1 9L9.21839 17"/></svg></button>',
nextArrow:'<button class="slider-btn slider-btn__right"><svg width="10" height="18" viewBox="0 0 10 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M0.78161 17L9 9L0.78161 1"/></svg></button>',
infinite:false,
});

  $('.questions__item-title').on('click', function(){
    $('.questions__item').removeClass('questions__item--active');
    $(this).parent().addClass('questions__item--active');
  });
  // $('#fullpage').fullpage({
  //   autoScrolling: true,
  //   sectionSelector:'.page-section',
  //   scrollOverflow:true,
  //   menu:'#header__nav',
  //   anchors:['top','products', 'benefits', 'specefications', 'questions', 'contacts']
  //     });

    // $('.menu__btn').on('click', function(){
    //     $('.menu__btn').toggleClass('menu__btn--active');
    //     $('.menu__btn').toggleClass('menu__list--active');
    // });


      
});
const menuBtn = document.querySelector('.menu__btn');
const menu = document.querySelector('.menu');
const menuList = document.querySelector('.menu__list');
const products = document.querySelector('.products') 
const bodyElement = document.querySelector('body');
document.addEventListener("click", show);
function show(event){
    if(event.target.closest('.menu__btn'))
    menuBtn.classList.toggle('_active');
    if(menuBtn.classList.contains('_active')){
      menu.classList.add('_active');
      bodyElement.classList.toggle('lock');
    }else{
      menu.classList.remove('_active');
    }
}

// // products.classList.add('_scroll');
// if(products.classList.contains('_scroll')){
// products.style.cssText='overflow:auto;'

// }
